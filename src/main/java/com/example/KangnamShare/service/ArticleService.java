...
public class ArticleService {
    ...
    public Posts show(Long id) {
        return articleRepository.findById(id).orElse(null);
    }

    public Posts create(ArticleDTO dto) {
        Posts posts = dto.toEntity();
        if(posts.getId() != null)
            return null;
        return articleRepository.save(posts);
    }

    public Posts update(Long id, ArticleDTO dto) {
        // 1: DTO -> 엔티티
        Posts posts = dto.toEntity();
        log.info("id: {}, article: {}", id, posts.toString());
        // 2: 타겟 조회
        Posts target = articleRepository.findById(id).orElse(null);
        // 3: 잘못된 요청 처리:json이 비어있거나 아이디가 다를 때
        if (target == null || id != posts.getId()) {
            // 400, 잘못된 요청 응답!
            log.info("잘못된 요청! id: {}, article: {}", id, posts.toString());
            return null;
        }
        //4:잘못된 요청 처리:게시글 작성자랑 수정하려는 사람이 다를 때
        if (!articleRepository.findUsernameByArticleId(id).equals(posts.getUsername())) {
            // 400, 잘못된 요청 응답!
            log.info("잘못된 요청! username: {}, article: {}", articleRepository.findUsernameByArticleId(id), posts.toString());
            return null;
        }
        // 4: 업데이트
        target.patch(posts);
        Posts updated = articleRepository.save(target);
        return updated;

    }

    public Posts delete(Long id, String username) {
        //대상찾기
        Posts target = articleRepository.findById(id).orElse(null);
    ...
    public List<Posts> createArticles(List<ArticleDTO> dtos) {
        //dto 묶음을 entity 묶음으로 변환
        List<Posts> postsList =dtos.stream()
